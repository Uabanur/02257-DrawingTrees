...
// Property 1
let nodesAtSameLevelShouldBeAtleastAGivenDistanceApart (config: DesignConfig) (tree:Tree<unit>) =
    let checkSpacing =
        floatsHasMinDifference config.Spacing
    ...
    in nodeDistanceCheck <| design config tree :: []
...
// Property 2
let parentIsCenteredOverOffsprings (config: DesignConfig) (tree: Tree<unit>) =
...
// Property 3
let treeHasReflectionalSymmetry (config: DesignConfig) (tree:Tree<unit>) =
    let rec mirrorTree (Node(v,c)) =
        Node(v, c |> List.map mirrorTree |> List.rev)

    let positionedOriginalTree = design config tree
    let positionedMirroredTree = design config (mirrorTree tree)
